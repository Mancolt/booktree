...
when every one of its files is in the same library folder, so an interrupted or split filing (discs left under
two different matches) is completed on the next run; symlinks in a library do not count. Nothing is ever deleted. Typical values are your
