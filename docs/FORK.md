...
| 10 | Audible/MAM metadata is remote input, yet a series/title/part of `..` passed `sanitize_filename` unchanged and became a path component; with a template of bare tokens it could leave the media root. Fixed in 3.0.4: `..`/`.` components become `_` and the target path is verified to lie under the media root | — |
